...
0. ~~给予本仓库一个 star（手动狗头）~~
1. 前往 [GitHub Releases](https://github.com/Steamauto/Steamauto/releases/latest) 下载适合自己系统的 Steamauto。
2. 运行一次程序，程序会自动生成 `config/config.json5` 和 `config/steam_account_info.json5`。
3. 编辑 `config/config.json5`，启用需要的插件和平台功能。
4. 编辑 `config/steam_account_info.json5`，填写 Steam 登录信息和令牌参数（获取方式见附录）。
5. 再次运行程序，并根据提示完成 BUFF / 悠悠有品等平台登录。
...
### `notify_service` 相关配置项说明

`notify_service` 是当前统一通知配置，不只限于 BUFF 插件。通知器使用 [Apprise](https://github.com/caronc/apprise) 格式，支持 Telegram、钉钉、飞书、WxPusher、Server 酱等。

| 配置项 | 描述 |
| --- | --- |
| `notifiers` | 通知服务器列表，填写 Apprise 格式地址；为空则不发送通知。 |
| `custom_title` | 自定义通知标题，为空则使用默认标题。 |
| `include_steam_info` | 是否在通知中包含 Steam 账号信息。 |
| `blacklist_words` | 通知屏蔽词；通知内容包含这些词时不会发送。 |

## 安全与排障

- 不要公开 `config/steam_account_info.json5`、`config/buff_cookies_*`、`config/uu_token_*`、`config/rsakey.txt`。
- 不要把包含账号、密码、token、cookie、私钥的配置截图或日志发送到公开 issue。
- 日志文件位于 `logs/`。遇到问题时请先查看最新日志；提交 issue 或在群内求助前，请删除敏感信息。
- `config/config.json5` 是 JSON5 文件，支持注释；部分编辑器把注释标成语法错误不代表程序无法读取。
...
请注意妥善保管本地配置、cookie、token、私钥和日志，不要将这些敏感信息公开给他人
...
推荐使用[ Watt Toolkit ](https://steampp.net/)获取Steam令牌参数 操作简便
...
2. 审核通过后流程
   ...
   2. 输入 RSA 公钥后，获取身份 ID；  // 备注: RSA 私钥在插件运行后需要填写进 `config/rsakey.txt`，请自行生成 RSA 密钥对，建议使用 2048 位或 4096 位密钥。如果你不会生成且不想学习，可以使用在线生成工具生成，例如 [https://rsagen.pages.dev/](https://rsagen.pages.dev/)（若使用此网站，请设置强度为 2048 或 4096，并自行确保浏览器环境安全）
