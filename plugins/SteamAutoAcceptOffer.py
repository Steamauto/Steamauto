            ...
            try:
                with self.steam_client_mutex:
                    if not self.steam_client.is_session_alive():
                        self.logger.info(f"[SteamAutoAcceptOffer] Steam账号{steam_username}： Steam会话已过期, 正在重新登录...")
                        self.steam_client.login(
                            self.steam_client.username, self.steam_client._password, json.dumps(self.steam_client.steam_guard)
                        )
                        self.logger.info(f"[SteamAutoAcceptOffer] Steam账号{steam_username}：  Steam会话已更新")
                with self.steam_client_mutex:
                    trade_summary = self.steam_client.get_trade_offers_summary()["response"]
                self.logger.info(f"[SteamAutoAcceptOffer] Steam账号{steam_username}：  检测到有%d个待处理的交易报价" % trade_summary["pending_received_count"])
                    ...
                    if len(trade_offers["trade_offers_received"]) > 0:
                        for trade_offer in trade_offers["trade_offers_received"]:
                            ...
                            if len(trade_offer.get("items_to_give", {})) == 0:
                                self.logger.info(
                                    f'f"[SteamAutoAcceptOffer] Steam账号{steam_username}：  检测到报价[{trade_offer["tradeofferid"]}]' f"属于礼物报价，正在接受报价..."
                                )
                                try:
                                    with self.steam_client_mutex:
                                    ...
                                    self.logger.error(f"[SteamAutoAcceptOffer] Steam账号{steam_username}：  mafile有问题, 请检查mafile是否正确" "(尤其是identity_secret)")
                                    self.logger.error(str(e))
                                except ConfirmationExpected:
                                    self.logger.error(f"[SteamAutoAcceptOffer] Steam账号{steam_username}：  Steam Session已经过期, 请删除session文件夹并重启Steamauto")
                                except Exception as e:
                                    self.logger.error(e, exc_info=True)
                                    self.logger.error(f"[SteamAutoAcceptOffer] Steam账号{steam_username}：  Steam异常! 稍后再试...")
                                self.logger.info(f'[SteamAutoAcceptOffer] Steam账号{steam_username}：  报价[{trade_offer["tradeofferid"]}]接受成功！')
                            else:
                                self.logger.info(
                                    f'[SteamAutoAcceptOffer] Steam账号{steam_username}：  检测到报价[{trade_offer["tradeofferid"]}]' f"需要支出物品，自动跳过处理"
                                )
            except Exception as e:
                self.logger.error(e, exc_info=True)
                self.logger.error(f"[SteamAutoAcceptOffer] Steam账号{steam_username}：  发生未知错误！稍后再试...")
