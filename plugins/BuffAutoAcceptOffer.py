...
class BuffAutoAcceptOffer:
    ...
    def __init__(self, logger, steam_client, steam_client_mutex, config):
        ...
        self.asset = AppriseAsset(
            plugin_paths=[
                os.path.join(os.path.dirname(__file__), "..", APPRISE_ASSET_FOLDER)
            ]
        )
        ...
        try:
            ...
            if resp.status_code == 200:
                if resp.json()["code"] == "OK":
                    ...
                    self.logger.error(
                        "[BuffAutoAcceptOffer] 开启买家发起交易报价功能失败"
                    )
        except:
    ...
    def get_order_info(self, trades):
        for trade in trades:
            if trade["tradeofferid"] not in self.order_info:
                if self.development_mode and os.path.exists(
                    SELL_ORDER_HISTORY_DEV_FILE_PATH
                ):
                    self.logger.info(
                        "[BuffAutoAcceptOffer] 开发者模式已开启, 使用本地数据"
                    )
                    with open(
                        SELL_ORDER_HISTORY_DEV_FILE_PATH,
                        "r",
                    ...
                    resp = requests.get(
                        sell_order_history_url, headers=self.buff_headers
                    )
                    resp_json = resp.json()
                    if self.development_mode:
                        self.logger.info(
                            "[BuffAutoAcceptOffer] 开发者模式, 保存交易历史信息到本地"
                        )
                        with open(
                            SELL_ORDER_HISTORY_DEV_FILE_PATH,
                            "w",
    ...
    def get_buff_bind_steamid(self):
        response_json = requests.get(
            "https://buff.163.com/account/api/user/info", headers=self.buff_headers
        ).json()
        if response_json["code"] == "OK":
            ...
            self.logger.error(
                "[BuffAutoOnSale] 获取BUFF绑定的SteamID失败, 请检查buff_cookies.txt或稍后再试! "
            )
            return ""
            ...
            if response_json["code"] == "OK":
                if "data" in response_json:
                    if "nickname" in response_json["data"]:
                        ...
                        if (
                            "data" not in steam_trade_response_json
                            or steam_trade_response_json["data"] is None
                        ):
                            self.logger.error(
                                "[BuffAutoAcceptOffer] BUFF账户登录状态失效, 请检查buff_cookies.txt或稍后再试! "
                            )
                            return ""
    ...
    def format_str(self, text: str, trade):
        for good in trade["goods_infos"]:
            ...
            created_at_time_str = time.strftime(
                "%Y-%m-%d %H:%M:%S", time.localtime(trade["created_at"])
            )
            text = text.format(
    ...
    def should_accept_offer(self, trade):
        ...
        protection_price_percentage = self.config["buff_auto_accept_offer"][
            "protection_price_percentage"
        ]
        ...
        if sell_protection:
            ...
            if (
                goods_id in self.lowest_on_sale_price_cache
                and self.lowest_on_sale_price_cache[goods_id]["cache_time"]
                >= datetime.datetime.now() - datetime.timedelta(hours=1)
            ):
                other_lowest_price = self.lowest_on_sale_price_cache[goods_id]["price"]
                self.logger.info(
                    "[BuffAutoAcceptOffer] 从缓存中获取最低价格: "
                    + str(other_lowest_price)
                )
            if other_lowest_price == -1:
                # 只检查第一个物品的价格, 多个物品为批量购买, 理论上批量上架的价格应该是一样的
                if trade["tradeofferid"] not in self.order_info:
                    self.logger.error(
                        "[BuffAutoAcceptOffer] 无法获取交易金额, 跳过此交易报价"
                    )
                    return False
                if self.development_mode and os.path.exists(SHOP_LISTING_DEV_FILE_PATH):
                    self.logger.info(
                        "[BuffAutoAcceptOffer] 开发者模式已开启, 使用本地价格数据"
                    )
                    with open(
                        SHOP_LISTING_DEV_FILE_PATH,
                        "r",
                    ...
                    if self.development_mode:
                        self.logger.info(
                            "[BuffAutoAcceptOffer] 开发者模式, 保存价格信息到本地"
                        )
                        with open(
                            SHOP_LISTING_DEV_FILE_PATH,
                            "w",
            ...
            if (
                price < other_lowest_price * protection_price_percentage
                and other_lowest_price > protection_price
            ):
    ...
    def exec(self):
        ...
        with open(
            BUFF_COOKIES_FILE_PATH, "r", encoding=get_encoding(BUFF_COOKIES_FILE_PATH)
        ) as f:
            ...
            try:
                with self.steam_client_mutex:
                    if not self.steam_client.is_session_alive():
                        self.logger.info(
                            "[BuffAutoAcceptOffer] Steam会话已过期, 正在重新登录..."
                        )
                        ...
                        steam_session_path = os.path.join(
                            SESSION_FOLDER, self.steam_client.username.lower() + ".pkl"
                        )
                        with open(steam_session_path, "wb") as f:
                            pickle.dump(self.steam_client.session, f)
                self.logger.info(
                    "[BuffAutoAcceptOffer] 正在进行BUFF待发货/待收货饰品检查..."
                )
                username = self.check_buff_account_state()
                if username == "":
                    self.logger.info(
                        "[BuffAutoAcceptOffer] BUFF账户登录状态失效, 尝试重新登录..."
                    )
                    if get_valid_session_for_buff(self.steam_client, self.logger) == "":
                        self.logger.error(
                            "[BuffAutoAcceptOffer] BUFF账户登录状态失效, 无法自动重新登录! "
                        )
                        if (
                            ...
                            for server in self.config["buff_auto_accept_offer"][
                                "servers"
                            ]:
                                apprise_obj.add(server)
                            apprise_obj.notify(
                                title=self.config["buff_auto_accept_offer"][
                                    "buff_cookie_expired_notification"
                                ]["title"],
                                body=self.config["buff_auto_accept_offer"][
                                    "buff_cookie_expired_notification"
                                ]["body"],
                            )
                        return
                if self.development_mode and os.path.exists(
                    ...
                    self.logger.info(
                        "[BuffAutoAcceptOffer] 开发者模式已开启, 使用本地消息通知文件"
                    )
                    with open(
                        MESSAGE_NOTIFICATION_DEV_FILE_PATH,
                        "r",
                        ...
                        to_deliver_order = message_notification["data"][
                            "to_deliver_order"
                        ]
                else:
                    response_json = requests.get(
                    ...
                    if self.development_mode:
                        self.logger.info(
                            "[BuffAutoAcceptOffer] 开发者模式, 保存发货信息到本地"
                        )
                        with open(
                            MESSAGE_NOTIFICATION_DEV_FILE_PATH,
                            "w",
                ...
                try:
                    if (
                        "csgo" in to_deliver_order
                        and int(to_deliver_order["csgo"]) != 0
                    ) or (
                        "dota2" in to_deliver_order
                        and int(to_deliver_order["dota2"]) != 0
                    ):
                        self.logger.info(
                            ...
                            + str(
                                (
                                    0
                                    if "csgo" not in to_deliver_order
                                    else int(to_deliver_order["csgo"])
                                )
                                + (
                                    0
                                    if "dota2" not in to_deliver_order
                                    else int(to_deliver_order["dota2"])
                                )
                            )
                            + "个待发货请求! "
                        )
                        self.logger.info(
                            "[BuffAutoAcceptOffer] CSGO待发货: "
                            + str(
                                (
                                    0
                                    if "csgo" not in to_deliver_order
                    ...
                    self.logger.error(
                        "[BuffAutoAcceptOffer] Buff接口返回数据异常! 请检查网络连接或稍后再试! "
                    )
                trade_supply = {}
                if self.development_mode and os.path.exists(STEAM_TRADE_DEV_FILE_PATH):
                    self.logger.info(
                        "[BuffAutoAcceptOffer] 开发者模式已开启, 使用本地待发货文件"
                    )
                    with open(
                    ...
                    if self.development_mode:
                        self.logger.info(
                            "[BuffAutoAcceptOffer] 开发者模式, 保存待发货信息到本地"
                        )
                        with open(
                            STEAM_TRADE_DEV_FILE_PATH,
                            "w",
                ...
                for game in SUPPORT_GAME_TYPES:
                    if self.development_mode and os.path.exists(
                        ...
                        self.logger.info(
                            "[BuffAutoAcceptOffer] 开发者模式已开启, 使用本地待确认供应文件"
                        )
                        with open(
                            TO_DELIVER_DEV_FILE_PATH.format(game=game["game"]),
                            "r",
                            encoding=get_encoding(TO_DELIVER_DEV_FILE_PATH).format(
                                game=game["game"]
                            ),
                        ) as f:
                        ...
                        if self.development_mode:
                            self.logger.info(
                                "[BuffAutoAcceptOffer] 开发者模式, 保存待确认供应信息到本地"
                            )
                            with open(
                                TO_DELIVER_DEV_FILE_PATH.format(game=game["game"]),
                                "w",
                        ...
                        for trade_offer in trade_supply[game["game"]]:
                            if (
                                trade_offer["tradeofferid"] is not None
                                and trade_offer["tradeofferid"] != ""
                            ):
                                trade_offer_to_confirm.add(trade_offer["tradeofferid"])
                        self.logger.info(
                            "[BuffAutoAcceptOffer] 为了避免访问接口过于频繁，休眠5秒..."
                        )
                        time.sleep(5)
                self.logger.info(
                    "[BuffAutoAcceptOffer] 查找到 "
                    + str(len(trades))
                    + " 个待处理的BUFF未发货订单! "
                )
                self.logger.info(
                    "[BuffAutoAcceptOffer] 查找到 "
                    + str(len(trade_offer_to_confirm) - len(trades))
                    + " 个待处理的BUFF待确认供应订单! "
                )
                ...
                try:
                            ...
                            if offer_id not in ignored_offer:
                                try:
                                    if not self.should_accept_offer(trade):
                                        continue
                                    self.logger.info(
                                        "[BuffAutoAcceptOffer] 正在检查报价物品..."
                                    )
                                    if not self.development_mode:
                                        with self.steam_client_mutex:
                                            offer = self.steam_client.get_trade_offer(
                                                offer_id
                                            )
                                        for item in offer["response"]["offer"][
                                            "items_to_give"
                                        ].values():
                                            match = False
                                            for item_in_trade in trade[
                                                "items_to_trade"
                                            ]:
                                            ...
                                            if not match:
                                                self.logger.error(
                                                    "[BuffAutoAcceptOffer] 报价中的物品不在待发货列表中, 跳过接受报价"
                                                )
                                                if (
                                        ...
                                        self.logger.info(
                                            "[BuffAutoAcceptOffer] 开发者模式已开启, 跳过报价物品检查"
                                        )
                                    self.logger.info(
                                        "[BuffAutoAcceptOffer] 报价物品检查完成! 正在接受报价..."
                                    )
                                    if self.development_mode:
                                        self.logger.info(
                                            "[BuffAutoAcceptOffer] 开发者模式已开启, 跳过接受报价"
                                        )
                                    else:
                                        try:
                                            with self.steam_client_mutex:
                    ...
                    for trade_offer_id in trade_offer_to_confirm:
                        if trade_offer_id not in ignored_offer:
                            if self.development_mode:
                                self.logger.info(
                                    "[BuffAutoAcceptOffer] 开发者模式已开启, 跳过令牌确认"
                                )
                            else:
                                with self.steam_client_mutex:
                                    offer = self.steam_client.get_trade_offer(
                                ...
                                if (
                                    "offer" in offer["response"]
                                    and "trade_offer_state"
                                    in offer["response"]["offer"]
                                ):
                                    if (
                                        offer["response"]["offer"]["trade_offer_state"]
                                        == 9
                                    ):
                                        with self.steam_client_mutex:
                                            self.steam_client._confirm_transaction(
                                                trade_offer_id
                                            )
                                        ignored_offer.append(trade_offer_id)
                                        self.logger.info(
                                            "[BuffAutoAcceptOffer] 令牌完成! ( "
                                            + trade_offer_id
                                            + " ) 已经将此交易报价加入忽略名单!"
                                        )
                                    else:
                            ...
                            if (
                                list(trade_offer_to_confirm).index(trade_offer_id)
                                != len(trade_offer_to_confirm) - 1
                            ):
                                self.logger.info(
                                    "[BuffAutoAcceptOffer] 为了避免频繁访问Steam接口, 等待5秒后继续..."
                                )
