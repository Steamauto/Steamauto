...
class BuffAutoAcceptOffer:
    ...
    def init(self) -> bool:
        ...
        session = get_valid_session_for_buff(self.steam_client, self.logger, proxies=proxies)
        self.buff_account = BuffAccount(session, proxies=proxies)
        try:
            username = self.buff_account.get_user_nickname()
            if username:
                self.logger = PluginLogger(f"BuffAutoAcceptOffer-{username}-steam:{self.steam_client.username}")
        except:
            pass
        ...
        if steam_info["max_bind_count"] == 1:
            ...
            for account in steam_info["items"]:
                if account["steamid"] == str(self.steam_client.get_steam64id_from_cookies()):
                    self.logger.info(f"检测到当前已经登录多个Steam账号，只会处理SteamID为{self.steam_client.get_steam64id_from_cookies()}的交易")
                    to_exit = False
                    break
        if to_exit:
            self.logger.error("当前登录的Steam账号不在BUFF账号绑定列表中，无法进行自动发货！")
            exit_code.set(1)
            return True
        return False
                            ...
                            for i, trade in enumerate(trades):
                                ...
                                if offer_id in ignored_offer:
                                    ...
                                    if ignored_offer[offer_id] > REPROCESS_THRESHOLD:
                                        self.logger.warning(f"报价 {offer_id} 已被忽略 {ignored_offer[offer_id] - 1} 次，超过阈值 {REPROCESS_THRESHOLD}，将尝试重新处理")
                                        del ignored_offer[offer_id]  # 从忽略字典中移除
                                        process_this_offer = True  # 标记需要处理
                                    else:
                                        self.logger.info("该报价已被处理过，跳过")
